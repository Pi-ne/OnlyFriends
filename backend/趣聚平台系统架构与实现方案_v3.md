...
趣聚平台采用**前后端分离 + 适度微服务**的整体架构。前端以微信小程序为主要用户入口，后台管理系统采用 Web 端（Vue 3）。后端按业务边界拆分为 **6 个独立微服务**：网关服务、用户服务、活动服务、社群服务、即时通讯服务和 AI 服务；后台管理功能作为独立的 Admin 服务，复用各业务服务的数据层能力。数据层以 MySQL 为主存储，Redis 承担缓存与会话管理，MinIO/OSS 承担文件与图片存储。
...
```
ququ-platform/
├── ququ-gateway/                   # API 网关服务
│   ├── src/main/java/com/ququ/gateway/
│   │   ├── config/                 # 路由配置、CORS 配置
│   │   ├── filter/                 # JWT 鉴权过滤器、限流过滤器
│   │   └── GatewayApplication.java
│   └── pom.xml
│
├── ququ-user-service/              # 用户服务
│   ├── src/main/java/com/ququ/user/
...
│   └── pom.xml
│
├── ququ-activity-service/          # 活动服务
│   ├── src/main/java/com/ququ/activity/
│   │   ├── controller/
│   │   ├── service/
...
│   └── pom.xml
│
├── ququ-social-service/            # 社群服务
│   ├── src/main/java/com/ququ/social/
│   │   ├── controller/
│   │   ├── service/
...
│   └── pom.xml
│
├── ququ-im-service/                # 即时通讯服务
│   ├── src/main/java/com/ququ/im/
...
│   └── pom.xml
│
├── ququ-admin-service/             # 后台管理服务
│   ├── src/main/java/com/ququ/admin/
│   │   ├── controller/
│   │   ├── service/
│   │   ├── feign/                  # 调用各业务服务
│   │   ├── entity/
...
│   └── pom.xml
│
├── ququ-common/                    # 公共模块（各服务依赖）
│   ├── src/main/java/com/ququ/common/
...
│   └── pom.xml
│
├── ququ-ai-service/                # AI 服务（FastAPI / Python）
│   ├── routers/
...
```
ququ-platform/                  ← 根仓库（Git 根目录）
├── ququ-gateway/               ← 网关服务（Java）
├── ququ-user-service/          ← 用户服务（Java）
├── ququ-activity-service/      ← 活动服务（Java）
├── ququ-social-service/        ← 社群服务（Java）
├── ququ-im-service/            ← IM 服务（Java）
├── ququ-admin-service/         ← 后台管理服务（Java）
├── ququ-common/                ← 公共模块（Java，被各服务依赖）
├── ququ-ai-service/            ← AI 服务（Python）
├── ququ-miniprogram/           ← 微信小程序（JS）
├── ququ-admin-web/             ← 后台管理 Web（Vue 3）
...
services:
  ...
  mysql:
    image: mysql:8.0
    container_name: ququ-mysql
    restart: unless-stopped
    environment:
      MYSQL_ROOT_PASSWORD: ququ_root_2024
      MYSQL_DATABASE: ququ_db
      MYSQL_USER: ququ
      MYSQL_PASSWORD: ququ_pass_2024
    ports:
      - "3306:3306"
    volumes:
    ...
    networks:
      - ququ-net
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-h", "localhost", "-uroot", "-pququ_root_2024"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  redis:
    ...
    container_name: ququ-redis
    restart: unless-stopped
    ports:
      - "6379:6379"
    volumes:
      - ./data/redis:/data
    command: redis-server --requirepass ququ_redis_2024 --appendonly yes
    networks:
      - ququ-net
    healthcheck:
      test: ["CMD", "redis-cli", "-a", "ququ_redis_2024", "ping"]
      interval: 10s
      timeout: 3s
      retries: 3
  ...
  nacos:
    ...
    environment:
      ...
      MYSQL_SERVICE_USER: ququ
      MYSQL_SERVICE_PASSWORD: ququ_pass_2024
  ...
  minio:
    ...
    container_name: ququ-minio
    restart: unless-stopped
    environment:
      MINIO_ROOT_USER: ququ_minio
      MINIO_ROOT_PASSWORD: ququ_minio_2024
    ports:
      - "9000:9000"   # API 端口
      - "9001:9001"   # 控制台端口
    volumes:
...
```bash
# 验证 MySQL
mysql -h 127.0.0.1 -P 3306 -u ququ -pququ_pass_2024 -e "SHOW DATABASES;"
# 预期：看到 ququ_db 和 nacos_config

# 验证 Redis
redis-cli -h 127.0.0.1 -a ququ_redis_2024 ping
# 预期：PONG
...
# 验证 MinIO（浏览器访问控制台）
# http://localhost:9001  账号 ququ_minio / ququ_minio_2024
```
...
| Bucket 名称 | 访问策略 | 用途 |
|-------------|----------|------|
| `ququ-avatars` | public（只读） | 用户头像 |
| `ququ-activity-images` | public（只读） | 活动图片/封面 |
| `ququ-licenses` | private | 商家营业执照 |
| `ququ-team-files` | private | 小队文件 |
| `ququ-team-album` | public（只读） | 小队相册 |
...
```
1. ququ-gateway          (端口 8080) — 最后启动
2. ququ-user-service     (端口 8081) — 先启动
3. ququ-activity-service (端口 8082)
4. ququ-social-service   (端口 8083)
5. ququ-im-service       (端口 8084)
6. ququ-admin-service    (端口 8085)
```
...
spring:
  ...
  datasource:
    url: jdbc:mysql://localhost:3306/ququ_db?useUnicode=true&characterEncoding=utf8mb4&serverTimezone=Asia/Shanghai
    username: ququ
    password: ququ_pass_2024
...
# JWT 配置（各服务保持一致）
jwt:
  secret: ququ_jwt_secret_key_2024_must_be_at_least_32_chars
...
minio:
  ...
  access-key: ququ_minio
  secret-key: ququ_minio_2024
  bucket-prefix: ququ-
...
在 `ququ-ai-service/` 目录下创建 `.env` 文件（不提交到 Git）：
...
REDIS_PORT=6379
REDIS_PASSWORD=ququ_redis_2024
...
CREATE DATABASE IF NOT EXISTS `ququ_db`
...
### C.1 父 POM（ququ-platform/pom.xml）
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
  ...
  <groupId>com.ququ</groupId>
  <artifactId>ququ-platform</artifactId>
  <version>1.0.0-SNAPSHOT</version>
  <packaging>pom</packaging>

  <modules>
    <module>ququ-common</module>
    <module>ququ-gateway</module>
    <module>ququ-user-service</module>
    <module>ququ-activity-service</module>
    <module>ququ-social-service</module>
    <module>ququ-im-service</module>
    <module>ququ-admin-service</module>
  </modules>
...
### C.3 用户服务（ququ-user-service）

**pom.xml 关键依赖：**

```xml
<dependencies>
  <dependency><groupId>com.ququ</groupId><artifactId>ququ-common</artifactId><version>1.0.0-SNAPSHOT</version></dependency>
...
### C.4 活动服务（ququ-activity-service）
...
### C.7 AI 服务（ququ-ai-service，Python）
...
app = FastAPI(title="趣聚 AI 服务", version="1.0.0")
...
```json
{
  ...
  "data": {
    ...
    "userInfo": {
      "userId": 10001,
      "nickname": "趣聚小达人",
      "avatarUrl": "http://minio/ququ-avatars/10001.jpg",
      "userType": 0,
      "creditScore": 100
    }
  },
...
```json
{
  ...
  "tags": ["徒步", "户外", "周末"],
  "coverUrl": "http://minio/ququ-activity-images/cover_001.jpg",
    ...
    "list": [
      {
        ...
        "status": 3,
        "statusText": "报名中",
        "creatorNickname": "趣聚小达人",
        "creatorAvatar": "http://minio/ququ-avatars/10001.jpg"
      }
    ]
  },
  "timestamp": 1700000000000
...
```json
{
  ...
  "data": {
    ...
    "rankList": [
      ...
      { "rank": 3, "userId": 10001, "nickname": "趣聚小达人", "avatarUrl": "...", "score": 85 }
    ]
  },
  "timestamp": 1700000000000
}
  ...
  "data": [
    {
      ...
      "targetUser": {
        "userId": 10002,
        "nickname": "山野小侠",
        "avatarUrl": "http://minio/ququ-avatars/10002.jpg"
      },
      "lastMsg": "明天几点出发？",
...
// 或 /topic/team/{teamId}（群聊）
{
  ...
  "convId": "10001_10002",
  "senderId": 10001,
  "senderNickname": "趣聚小达人",
  "senderAvatar": "http://minio/ququ-avatars/10001.jpg",
  "msgType": 1,
  "content": "明天几点出发？",
  "mediaUrl": null,
  "isRecalled": false,
...
**被调用方：** `ququ-user-service`，实现于 `UserInternalController.java`

**调用方：** 活动服务、社群服务、IM 服务、后台管理服务

```java
// 文件路径：ququ-user-service/src/main/java/com/ququ/user/controller/internal/UserInternalController.java
@RestController
...
**调用方 Feign Client 定义（放在 ququ-common 或各服务的 feign 包中）：**

```java
// 文件路径：ququ-activity-service/src/main/java/com/ququ/activity/feign/UserFeignClient.java
...
**被调用方：** `ququ-social-service`，实现于 `SocialInternalController.java`

**调用方：** IM 服务（验证消息发送权限）、后台管理服务（查询小队信息）

```java
// 文件路径：ququ-social-service/src/main/java/com/ququ/social/controller/internal/SocialInternalController.java
@RestController
@RequestMapping("/internal/social")
public class SocialInternalController {
...
```java
// 文件路径：ququ-im-service/src/main/java/com/ququ/im/feign/SocialFeignClient.java
...
```java
// 文件路径：ququ-activity-service/src/main/java/com/ququ/activity/feign/AiFeignClient.java
