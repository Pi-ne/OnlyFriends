...
mvn -f ququ-user-service/pom.xml spring-boot:run
mvn -f ququ-activity-service/pom.xml spring-boot:run
mvn -f ququ-social-service/pom.xml spring-boot:run
mvn -f ququ-im-service/pom.xml spring-boot:run
mvn -f ququ-admin-service/pom.xml spring-boot:run
mvn -f ququ-ai-service/pom.xml spring-boot:run
mvn -f ququ-gateway/pom.xml spring-boot:run
```
...
```powershell
docker exec -i ququ-mysql mysql -uroot -pququ_root_password --default-character-set=utf8mb4 -N -e "SELECT activate_token FROM ququ_user.user WHERE email='demo.a@example.com';"
```
...
```powershell
docker exec -i ququ-mysql mysql -uroot -pququ_root_password --default-character-set=utf8mb4 -N -e "SELECT activate_token FROM ququ_user.user WHERE email='demo.b@example.com';"
```
