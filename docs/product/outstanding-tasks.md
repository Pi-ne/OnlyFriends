...
| 用户服务 | 较高 | 注册登录资料商家、微信登录 Mock、后台内部接口较完整 |
| 活动服务 | 较高 | 核心流程完整；已补通知、克隆、标签、候补超时、队内权限 |
| 社群服务 | 较高 | 小队主流程完整，好友备注/分组已补，部分专用记录表未落地 |
| IM 服务 | 中高 | 聊天核心可用，统一 `im_message`；群聊标题等待增强 |
| AI 服务 | 中 | 接口完整，真实 AI 能力未接入 |
| 后台管理 | 中高 | 管理功能可用，OpenFeign 聚合，审计查询接口不足 |
| 小程序前端 | 中高 | 活动主链路、社群列表、IM 私聊、商家申请、签到等已联调 |
| Web 管理台 | 较高 | 7 视图含双用户好友/小队群聊联调 |
| 数据库/部署 | 中 | init-all 完整度较好；Flyway 策略与 IDE 直连密码需留意 |

## 7. 一句话总结

当前代码可支撑「后端核心流程 + 小程序主链路 + 管理台全功能」的演示版本。后续重点：真实 AI 接入、通知实时推送、专用操作记录落库、审计查询 UI、生产部署与内网隔离。
