...
- Activity templates in `ququ_activity.activity_template`.
- Default admin account in `ququ_admin.admin_user`.
...
User service database `ququ_user`:
...
Activity service database `ququ_activity`:
...
Social service database `ququ_social`:
...
IM service database `ququ_im`:

- `im_conversation`
- `im_message`
- `im_conversation_read`

Admin service database `ququ_admin`:
...
- The AI mock service does not own a separate database table in the current codebase. Activity AI review results are persisted in `ququ_activity.activity_review_record`.
